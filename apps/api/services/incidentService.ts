...
export class IncidentService {
  ...
  async createIncident(
    ...
    const slackConnection = user.Connections.find((data) => data.Slack)?.Slack;
    ...
    if (discordConnection) {
      if (discordConnection?.url) {
        ...
        await postMessageToDiscord(discordConnection.url, messageContent);
      }
    }
    ...
    const emailConnection = user.Connections.find((data) => data.email)?.email;
    ...
    if (emailConnection) {
      ...
      if (recipientEmail) {
        ...
        try {
          ...
          console.log(`Incident notification email sent to ${recipientEmail}`);
        } catch (emailError) {
          console.error(
